
=== DTRT WP Plugin ===
Contributors: dotherightthingnz
Donate link: http://dotherightthing.co.nz
Tags: boilerplate,
Requires at least: 4.8.1
Tested up to: 4.9.5
Requires PHP: 5.6.30
Stable tag: 1.4.15
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Base classes for a WordPress plugin and associated shortcodes and widgets.

== Description ==

Base classes for a WordPress plugin and associated shortcodes and widgets.

== Installation ==

Please install and use via the [WordPress Plugin Boilerplate Generator](https://github.com/dotherightthing/generator-wp-plugin-boilerplate)

== Changelog ==

= 1.4.15 =
* Provide fallback if no plugin data passed in 
* Add plugin options to test plugin
* Add placeholder test for Settings page which outputs PHP warnings 

= 1.4.14 =
* Update gulp-wpdtrt-plugin-bump
* Fix paths for gulp-wpdtrt-plugin-bump

= 1.4.13 =
* Update gulp-wpdtrt-plugin-bump

= 1.4.12 =
* Update demo strings
* Move bump_replace functionality to gulp-wpdtrt-plugin-bump

= 1.4.11 =
* gulp bump - Allow for up to 3 digits in each of the major/minor/patch semver numbers

= 1.4.10 =
* Data options can't be altered by the user so replace rather than merge them
* Move unset option to unit tests
* Don't update options if new options weren't passed in
* Set last_updated without affecting force_refresh
* Check for force_refresh before using it
* Don't set default data options
* Make unset_options available to unit tests

= 1.4.9 =
* Replace instance options rather than merging them, as the values are not stored as config options

= 1.4.8 =
* Push each additional plugin dependency into the existing array, rather than merging it
* Treat the raw config as the canonical source for plugin options, only merging in any user values

= 1.4.7 =
* When merging arrays, remove old/new duplicates and don't assume that nested arrays exist

= 1.4.6 =
* gulp bump - Bump constant for any child plugin name

= 1.4.5 =
* gulp bump - bump child plugin version
* gulp bump - ensure that composer update has finished before reading package.json
* gulp bump - make task header more readable

= 1.4.4 =
* gulp bump - refactor gulp bump
* gulp bump - get version number from correct copy of package.json
* gulp bump - output changed files to correct directory

= 1.4.3 =
* gulp bump - perform both replacements in Plugin.php

= 1.4.2 =
* gulp bump - bump plugin version
* gulp bump - bump autoloaded version number
* gulp bump - regenerate autoload files
* gulp bump - bump wpdrt-plugin version in child plugin
* gulp - fix taskheader message check

= 1.4.1 =
* Fix library link output on Plugins page
* 'Namespace' CSS, style settings page field hint
* Add gulp bump to update the version of wpdtrt-plugin files, using the version in package.json

= 1.4.0 =
* Add widgets include to test plugin
* Append release number to namespace to allow plugins to use different versions of wpdtrt-plugin, as necessary
* Add messages required by shortcode demo
* Track version of parent plugin
* Prefix wp_ajax callback, to call the refresh_api_data method of the correct plugin instance

= 1.3.6 =
* Display DTRT WP Plugin version on Plugins page
* Documentation

= 1.3.5 =
* Merge in get_api_data() functionality from child plugin method
* Get API $endpoint via a WordPress filter called by the child theme
* Add unit test for get_api_data()

= 1.3.4 =
* Add a plugin 'Settings' link to the plugins page

= 1.3.3 =
* Add CSS fieldset reset
* Set force_refresh fallback in case options form not submitted yet

= 1.3.2 =
* Pass real shortcode 'name' to demo shortcode
* Support mock data in demo shortcode
* Remove redundant check for duplicate merged options
* Documentation

= 1.3.1 =
* Fixed merging/persistence of plugin dependencies

= 1.3.0 =
* Fixed merging/persistence of plugin options

= 1.2.31 =
* Add common backend styles

= 1.2.30 =
* Migrate from Bower & NPM to Yarn

= 1.2.24 =
* Fix release.zip not being generated by Travis

= 1.2.23 =
* Improve SCSS style management through the use of variables and extends

= 1.2.20 =
* Make any content between the shortcode tags available to the shortcode template
* Use Private Packagist (premium), falls back to original sources

= 1.2.19 =
* Make any content between the shortcode tags available to the shortcode template

= 1.2.18 =
* Install test framework via Travis 

= 0.1 =
* Initial version

== Upgrade Notice ==

= 0.1 =
* Initial release
